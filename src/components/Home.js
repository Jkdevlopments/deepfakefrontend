...
const socket = io("https://cordately-unlustful-aidyn.ngrok-free.dev", {
  transports: ['websocket'], // 'polling' use pannaadhinga ngrok-la
  withCredentials: true,
  extraHeaders: {
    "ngrok-skip-browser-warning": "69420" // Ngrok warning-ai automatic-aga skip panna
  }
});
...
const Home = ({ user, setUser }) => {
  ...
  `https://cordately-unlustful-aidyn.ngrok-free.dev/api/users/all/${user.id}`,
  {
    headers: {
          ...
          data.users.map(async (u) => {
            try {
              const lastMsgRes = await fetch(
                `https://cordately-unlustful-aidyn.ngrok-free.dev/api/messages/last/${user.id}/${u.id}`
              );
  ...
  const deleteSelectedMessages = async () => {
    ...
    try {
      const response = await fetch('https://cordately-unlustful-aidyn.ngrok-free.dev/api/messages/delete-for-me', {
        method: 'POST',
        headers: {
  ...
  const endCall = useCallback((fromOther = false) => {
    ...
    if (isEndingCall) return; // Prevent multiple end calls
    setIsEndingCall(true);
    
    cleanupCallResources();

    // Notify the other person only if this is not already from other
    if (!fromOther) {
      let targetId = null;
      if (activeCall?.with) {
  ...
  useEffect(() => {
    ...
    socket.on("receiveMessage", (data) => {
      console.log("📩 Message received with analysis:", data.analysis);
      ...
      if (data.receiverId === user.id) {
        const isSelectedChat = selectedChat?.id === data.senderId;
        ...
        setMessagesByChat((prev) => {
          ...
          return { ...prev, [data.senderId]: [...chatMessages, newMsg] };
        });
        ...
        updateChatOrder(data.senderId, lastMessageText, new Date().toISOString());
    ...
    // FIXED: Handle call acceptance on caller side
    socket.on("callAccepted", async (data) => {
      console.log("📞 Call accepted by receiver");
      if (outgoingCall && peerConnection) {
        try {
          await peerConnection.setRemoteDescription(new RTCSessionDescription(data.answer));
          // Set active call on caller side
          setActiveCall({
    ...
    // FIXED: Handle call ended from other side
    socket.on("callEnded", () => {
  ...
  const fetchMessages = async (receiverId) => {
    try {
      const res = await fetch(
        `https://cordately-unlustful-aidyn.ngrok-free.dev/api/messages/${user.id}/${receiverId}`
      );
      const data = await res.json();
      ...
      const formatted = data.messages.map((msg) => ({
        ...
        analysis:
  typeof msg.analysis_result === "string"
    ? JSON.parse(msg.analysis_result)
    : msg.analysis_result || null,
      ...
      console.log("Fetched messages with analysis:", formatted);
      setMessages(formatted);
      setMessagesByChat((prev) => ({ ...prev, [receiverId]: formatted }));
  ...
  const createNewChat = async (receiverId) => {
    try {
      await fetch('https://cordately-unlustful-aidyn.ngrok-free.dev/api/chats/create', {
        method: 'POST',
  ...
  const sendVoiceMessage = async () => {
    ...
    try {
      ...
      const res = await fetch("https://cordately-unlustful-aidyn.ngrok-free.dev/api/analyze-media", {
        method: "POST",
        body: formData,
      });
      ...
      if (data.success) {
        ...
        if (data.analysis) {
          const isDeepfake = data.analysis.audio_analysis?.fake_prob > 50;
          if (isDeepfake) {
  ...
  const sendMediaMessage = async () => {
    ...
    updateChatOrder(selectedChat.id, '📎 Media', currentTime.toISOString());
    ...
    try {
      ...
      const data = await res.json();
      console.log("✅ Media analysis complete:", data);
      ...
      if (data.success) {
        ...
        if (data.analysis) {
          const isDeepfake = 
            (data.analysis.video_analysis?.score > 50) ||
            (data.analysis.audio_analysis?.fake_prob > 50);
          
          if (isDeepfake) {
            showToast("⚠️ Deepfake detected in media!", "warning");
          } else {
            showToast("✅ Media is real", "success");
          }
        }
      }
  ...
  const handleLogout = async () => {
    try {
      ...
      await fetch("https://cordately-unlustful-aidyn.ngrok-free.dev/api/logout", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          userId: user.id,
  ...
  const renderFileWithAnalysis = (fileUrl, fileType, fileName, analysis, status) => {
    ...
    const isAudio = fileType?.startsWith('audio/') || 
                   ...
                   fileUrl.match(/\.(mp3|wav|ogg|m4a|aac)(\?.*)?$/i);
    ...
    const renderAnalysisBadge = () => {
      ...
      const isDeepfake = (
        (analysis.video_analysis?.score > 50) ||
        (analysis.audio_analysis?.fake_prob > 50)
      );

      return (
        <div 
          className={`analysis-badge ${isDeepfake ? 'deepfake' : 'real'}`}
  ...
  return (
    <div className="home-container">
      ...
      {selectedAnalysis && (
        <div className="modal-overlay" onClick={() => setSelectedAnalysis(null)}>
          <div className="analysis-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal-body">
              {selectedAnalysis.video_analysis && (
                <div className="analysis-section">
                  <h4>Video Analysis</h4>
                  <div className="analysis-grid">
                    <div className="analysis-item">
                      <span>Score:</span>
                      <span>{selectedAnalysis.video_analysis.score}%</span>
                    </div>
                    <div className="analysis-item">
                      <span>Verdict:</span>
                      <span>{selectedAnalysis.video_analysis.verdict}</span>
                    </div>
                  </div>
                </div>
              )}
              {selectedAnalysis.audio_analysis && (
                <div className="analysis-section">
                  <h4>Audio Analysis</h4>
                  <div className="analysis-grid">
                    <div className="analysis-item">
                      <span>Fake:</span>
                      <span>{selectedAnalysis.audio_analysis.fake_prob}%</span>
                    </div>
                    <div className="analysis-item">
                      <span>Real:</span>
                      <span>{selectedAnalysis.audio_analysis.real_prob}%</span>
                    </div>
                    <div className="analysis-item">
                      <span>Verdict:</span>
                      <span>{selectedAnalysis.audio_analysis.verdict}</span>
                    </div>
                  </div>
                </div>
              )}
      ...
      <div className="main-content">
              ...
              filteredUsers.map((u) => (
                <div
                  ...
                  onClick={() => {
                    setSelectedChat(u);
                    createNewChat(u.id);
                    if (messagesByChat[u.id]) {
                      setMessages(messagesByChat[u.id]);
                    } else {
                      fetchMessages(u.id);
                    }
        ...
        {/* Chat Section - Hidden on mobile when sidebar is open */}
            ...
            <>
              ...
              <div className="chat-footer">
                ...
                <div className="chat-input-container">
                      ...
                      {showAttachMenu && (
                        <div className="attach-menu">
                          ...
                          <button className="attach-option" onClick={() => {
                            ...
                            <span className="attach-icon">🎤</span>
                            <span className="attach-label">Voice</span>
                          </button>
                        </div>
                      )}
                    </div>
                  ...
                  <button 
                    className={`send-btn ${(!input.trim() && !selectedFile && !audioBlob) || isUploading ? 'disabled' : ''}`}
                    onClick={handleSend}
                    disabled={(!input.trim() && !selectedFile && !audioBlob) || isUploading}
                  >
                    {isUploading ? '⏳' : '➤'}
                  </button>
                </div>
